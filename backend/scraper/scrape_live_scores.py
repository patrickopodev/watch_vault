...
def ensure_table():
    if not SUPABASE_URL or not SUPABASE_KEY:
        return
    sql = "CREATE TABLE IF NOT EXISTS public.live_scores (id bigint generated by default as identity primary key, match_id text not null unique, sport text not null, home_team text not null, away_team text not null, home_score text, away_score text, minute text, status text not null default 'scheduled', league text, scraped_at timestamptz not null default now());"
    headers = {
        "apikey": SUPABASE_KEY,
        "Authorization": f"Bearer {SUPABASE_KEY}",
        "Content-Type": "application/json",
    }
    paths = ["/pg-meta/query", "/pg-meta/default/query", "/pg-meta"]
    ref = SUPABASE_URL.rstrip("/").split("//")[-1].split(".")[0]
    mgmt_url = f"https://api.supabase.com/v1/projects/{ref}/database/query"
    try:
        resp = requests.post(mgmt_url, json={"query": sql}, headers=headers, timeout=10)
        if resp.ok:
            print(f"  table created via management API ({resp.status_code})")
            return
    except Exception as e:
        print(f"  mgmt API failed: {e}")
    for p in paths:
        try:
            resp = requests.post(f"{SUPABASE_URL}{p}", json={"query": sql}, headers=headers, timeout=10)
            if resp.ok:
                print(f"  table created via {p}")
                return
            print(f"  {p}: {resp.status_code} {resp.text[:100]}")
        except Exception as e:
            print(f"  {p}: {e}")
    print("  WARNING: could not create table, will try inserts anyway")
